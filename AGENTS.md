...
│   └── responsesTypes.ts                  # Responses API 类型定义
├── gitCommit/
...
| `responses/responsesApi.ts`           |       | Responses API `input`/`instructions` 转换、`/responses` 请求和非流式 JSON 响应处理                                                                                                                          |
| `responses/responsesTypes.ts`         |       | Responses API 请求、输入项和响应类型                                                                                                            |
...
将 VS Code 消息转换为 OpenAI 格式（**异步**）。支持文本、图片、工具调用、工具结果、推理内容的消息转换。modelConfig 新增 `vision` 字段，非视觉模型时自动替换图片为文本引用并存储图片数据；**`tool` 角色消息的 `content` 恒为纯字符串**——视觉模型时工具结果（`LanguageModelToolResultPart`）内的图片 `LanguageModelDataPart`（如内置 `view_image`、浏览器截图等 MCP 工具返回的图片）被抽取出来，在对应 `tool` 消息之后以一条追加的 `user` 消息（`image_url` content 块）发送，`tool` 消息内仅留下说明文本（"The tool returned N image(s). They are attached in the next user message."），因为 OpenAI 兼容上游（Console Go）要求 `tool.content` 为纯字符串，多模态数组会被 422 拒绝（`tool.content.str - Input should be a valid string`）；**MCP 工具返回的 resource-link（`application/vnd.code.resource-link`）data part 会被解析并通过 `resolveResourceLinkToImage()` 读取为实际图片，视觉模型经上述 user 消息路径发送、非视觉模型存入 `_localImages` 供 `ask_image` 代理使用，解析失败时以文本形式提示 URI**。
